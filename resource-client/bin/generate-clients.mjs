 ...
 * Types: postgresql | mssql | dynamodb | cosmosdb | snowflake | bigquery | neo4j | hubspot | googlecalendar | googlesheets | outreach | custom | graphql | lambda | salesforce | s3 | slack | majorauth | googleanalytics | quickbooks | gong | dynamics | linear | ringcentral | linkedinads | zohodesk | zohoprojects | sqs
 *
 * Examples:
...
function getClientClass(type) {
  const typeMap = {
    ...
    'linkedinads': 'LinkedInAdsResourceClient',
...
function addResource(resourceId, name, type, description, applicationId, framework, mode) {
  const validTypes = ['postgresql', 'mssql', 'dynamodb', 'cosmosdb', 'snowflake', 'bigquery', 'neo4j', 'hubspot', 'googlecalendar', 'googlesheets', 'outreach', 'custom', 'graphql', 'lambda', 'salesforce', 's3', 'slack', 'majorauth', 'googleanalytics', 'quickbooks', 'gong', 'dynamics', 'linear', 'ringcentral', 'linkedinads', 'zohodesk', 'zohoprojects', 'sqs'];
