...
function generateFromBindings(framework) {
  ...
  // slots that no longer exist without clobbering any module the author hand-wrote here.
  const GENERATED_MARKER = 'Auto-generated by @major-tech/resource-client';
  for (const f of fs.readdirSync(clientsDir)) {
    const p = path.join(clientsDir, f);
    if (fs.statSync(p).isFile() && fs.readFileSync(p, 'utf-8').includes(GENERATED_MARKER)) {
      fs.unlinkSync(p);
    }
  }
  ...
  fs.writeFileSync(path.join(clientsDir, 'index.ts'), bindingsIndexTemplate(exports), 'utf-8');
