...
function applyTextFilter(rows: User[], field: keyof User, params: URLSearchParams, paramName: string): User[] {
	...
	switch (op) {
		case "eq": return rows.filter((u) => String(u[field]).toLowerCase() === query);
		case "contains": return rows.filter((u) => String(u[field]).toLowerCase().includes(query));
		case "starts_with": return rows.filter((u) => String(u[field]).toLowerCase().startsWith(query));
		case "ends_with": return rows.filter((u) => String(u[field]).toLowerCase().endsWith(query));
		case "neq": return rows.filter((u) => String(u[field]).toLowerCase() !== query);
		default: return rows;
	}
}
...
function handleUsersRequest(searchParams: URLSearchParams, errorOnPage?: string) {
	...
	if (errorOnPage && page === parseInt(errorOnPage, 10)) {
		return { success: false, error: { code: "SIMULATED_ERROR", message: `Server error loading page ${page}. Please try again.` } };
	}
	...
	if (search) {
		...
		result = result.filter(
			(u) => u.name.toLowerCase().includes(query) || u.email.toLowerCase().includes(query) || u.department.toLowerCase().includes(query),
		);
	}
	...
	return { success: true, items, page, totalPages, totalCount };
}
	...
	return {
		...
		configureServer(server) {
			server.middlewares.use("/api/users", (req, res) => {
				...
				const errorOnPage = url.searchParams.get("errorOnPage") ?? undefined;
				const body = handleUsersRequest(url.searchParams, errorOnPage);
